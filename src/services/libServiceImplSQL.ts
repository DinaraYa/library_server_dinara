...
export class LibServiceImplSQL implements LibService {
    ...
    async pickUpBook(id: string, reader: string): Promise<void> {
        await configuration.pool.query('UPDATE books SET status = ? WHERE id = ?', [BookStatus.ON_HAND, id]);
        await configuration.pool.query('UPDATE readers_books SET pick_date = ? WHERE id_book = ?', [new Date().toDateString(), id]);
        await configuration.pool.query('UPDATE readers SET reader_name = ? WHERE id_reader = ?', [reader])
        return Promise.resolve(undefined);
    }

    async returnBook(id: string): Promise<void> {
        await configuration.pool.query('UPDATE books  SET status = ? WHERE id = ?', [BookStatus.ON_STOCK, id]);
        await configuration.pool.query('UPDATE readers_books SET return_date = ? WHERE id_book =?', [new Date().toDateString(), id])
        return Promise.resolve(undefined);
    }
    ...
    async removeBook(id: string): Promise<Book> {
        ...
        const removeBook = await configuration.pool.query('DELETE FROM books WHERE id = ?', [id]);
        return book;
    }
